...
var DateTimeShortcuts = {
    ...
    init: function() {
        ...
        for (var i=0; i<scripts.length; i++) {
            if (scripts[i].src.match(/DateTimeShortcuts/)) {
                var idx = scripts[i].src.indexOf('cms/DateTimeShortcuts');
    ...
    addClock: function(inp) {
        ...
        quickElement('img', clock_link, '', 'src', DateTimeShortcuts.admin_media_prefix + 'cms/img/icon_clock.gif', 'alt', gettext('Clock'));
    ...
    addCalendar: function(inp) {
        ...
        quickElement('img', cal_link, '', 'src', DateTimeShortcuts.admin_media_prefix + 'cms/img/icon_calendar.gif', 'alt', gettext('Calendar'));
        shortcuts_span.appendChild(document.createTextNode('\240'));
    ...
    openCalendar: function(num) {
        ...
        // is it left-to-right or right-to-left layout ?
        if (getStyle(document.body,'direction')!='rtl') {
            var x = findPosX(cal_link) + 17;
            if (window.outerWidth < x + 180) {
                x = window.outerWidth - 200;
            }
            cal_box.style.left = x + 'px';
        }
        else {
            // since style's width is in em, it'd be tough to calculate
            // px value of it. let's use an estimated px for now
