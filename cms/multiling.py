...
class MultilingualModel(models.Model):
    ...
    def __getattr__(self, attr, fail_silently=MULTILINGUAL_FAIL_SILENTLY):
        ...
        for field in self._meta.multilingual:
            ...
            if code is not None:
                try: 
                    return self._meta.translation.objects.select_related().get(model=self, language=code).__dict__[field]
                except ObjectDoesNotExist:
                    if MULTILINGUAL_FALL_BACK_TO_DEFAULT and MULTILINGUAL_DEFAULT and code != MULTILINGUAL_DEFAULT:
                        try:
                            return self._meta.translation.objects.select_related().get(model=self, language=MULTILINGUAL_DEFAULT).__dict__[field]
                        except ObjectDoesNotExist:
                            pass
                    if fail_silently:
                        return None
    ...
    def get_translated_or_none(self, attr):
        return  self.__getattr__(attr, fail_silently=True)
